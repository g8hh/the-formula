addLayer("c", {
    ...
    hoursPerDay() { 
        ...
        let day = Decimal.pow(1.1, l).times(l.plus(1).times(10))
    ...
    clockRatio() { 
        ...
        return player.points.sub(player.c.nullPoints).max(0).times(getTimeSpeed()).div(day).max(0);
    },
    displayedTime() {
    ...
    update(diff) {
        ...
        if (tmp.c.clockUnl) {
            ...
            if (tmp.c.clockRatio.gte(1)) {
                player.c.totalLoops = player.c.totalLoops.plus(1);
